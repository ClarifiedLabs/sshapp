...
run_unit_tests() {
  ...
  for attempt in $(seq 1 "$UNIT_TEST_ATTEMPTS"); do
    destination="$(resolve_unit_destination)"
    result_bundle="$XCODE_RESULT_BUNDLE_PATH/unit-tests-attempt-${attempt}.xcresult"

    if run_test_target "unit tests attempt $attempt/$UNIT_TEST_ATTEMPTS" "SSHAppTests" "$destination" "$result_bundle"; then
      return 0
    fi

    if [[ "$attempt" -lt "$UNIT_TEST_ATTEMPTS" ]]; then
      echo "Unit tests failed on attempt $attempt; retrying with a fresh simulator."
    fi
  done
